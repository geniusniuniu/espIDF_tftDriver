...
static esp_err_t panel_ili9341_del(void);
static esp_err_t panel_ili9341_reset(void);
static esp_err_t panel_ili9341_init(void);
static esp_err_t panel_ili9341_invert_color( bool invert_color_data);
static esp_err_t panel_ili9341_mirror(bool mirror_x, bool mirror_y);
static esp_err_t panel_ili9341_swap_xy(bool swap_axes);
static esp_err_t panel_ili9341_set_gap(int x_gap, int y_gap);
static esp_err_t panel_ili9341_disp_on_off(bool off);
...
static esp_err_t panel_ili9341_reset(void)
{
 //   ili9341_panel_t *ili9341 = __containerof(panel, ili9341_panel_t, base);
    ...
    if (ili9341->reset_gpio_num >= 0) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, LCD_CMD_SWRESET, NULL, 0), TAG, "send command failed");
...
static esp_err_t panel_ili9341_init()
{
 //   ili9341_panel_t *ili9341 = __containerof(panel, ili9341_panel_t, base);
    // esp_lcd_panel_io_handle_t io = ili9341->io;

    // LCD goes into sleep mode and display will be turned off after power on reset, exit sleep mode first
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, LCD_CMD_SLPOUT, NULL, 0), TAG, "send command failed");
    vTaskDelay(pdMS_TO_TICKS(100));
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, LCD_CMD_MADCTL, (uint8_t[]) {
        ili9341->madctl_val,
    }, 1), TAG, "send command failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, LCD_CMD_COLMOD, (uint8_t[]) {
        ili9341->colmod_val,
    }, 1), TAG, "send command failed");
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG, "send command failed");
...
esp_err_t panel_ili9341_draw_bitmap(int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    // ili9341_panel_t *ili9341 = __containerof(panel, ili9341_panel_t, base);
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, LCD_CMD_CASET, (uint8_t[]) {
        (x_start >> 8) & 0xFF,
        x_start & 0xFF,
        ((x_end - 1) >> 8) & 0xFF,
        (x_end - 1) & 0xFF,
    }, 4), TAG, "send command failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, LCD_CMD_RASET, (uint8_t[]) {
        (y_start >> 8) & 0xFF,
        y_start & 0xFF,
        ((y_end - 1) >> 8) & 0xFF,
        (y_end - 1) & 0xFF,
...
static esp_err_t panel_ili9341_invert_color(bool invert_color_data)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(ili9341->io, command, NULL, 0), TAG, "send command failed");
    return ESP_OK;
}
...
static esp_err_t panel_ili9341_disp_on_off(bool on_off)
{
    // ili9341_panel_t *ili9341 = __containerof(panel, ili9341_panel_t, base);
    // esp_lcd_panel_io_handle_t io = ili9341->io;
    int command = 0;
